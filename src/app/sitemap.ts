...
export async function generateSitemaps() {
    try {
        ...
        // Fallback to a single sitemap on error to avoid breaking the build/runtime
        return [{ id: 0 }];
    }
}
...
export default async function sitemap({ id }: { id: number }): Promise<MetadataRoute.Sitemap> {
    ...
    try {
        ...
        if (chunkId === 0) {
            let industryUrls: MetadataRoute.Sitemap = [];
            try {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                industryUrls = industries.map((ind: any) => ({
                ...
                console.error("Error fetching industries for sitemap:", indError);
                // Continue without industries if fetch fails
            }
        ...
        console.error(`Error generating sitemap id ${id}:`, error);

        // If this is the main sitemap (id 0), we must return AT LEAST the static pages
        // otherwise the sitemap is completely broken
        if (chunkId === 0) {
            return [
                {
                    url: BASE_URL,
        ...
        // For other chunks, return empty if failed
        return [];
    }
}
